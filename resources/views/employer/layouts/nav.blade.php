...
<div class="flex items-center gap-8 text-gray-700">
    ...
    <a href="{{ route('employer.temukan-kandidat') }}"
        class="hover:text-blue-600 cursor-pointer" title="Cari Kandidat">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 2xl:w-8 2xl:h-8" fill="none"
    ...
    @php
        ...
        $unreadCount = $employer ? EmployerNotification::where('employer_id', $employer->id)->where('is_read', false)->count() : 0;
    @endphp
    ...
    <a href="{{ route('employer.edit-profile', ['slug' => auth()->user()->employer->slug]) }}"
        ...
        @if ($photo)
            <img src="{{ asset('storage/' . $photo) }}" alt="Foto Profil"
