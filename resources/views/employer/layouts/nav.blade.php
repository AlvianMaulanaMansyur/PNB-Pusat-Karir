<div class="px-6 lg:px-20 mt-4">
    <div class="flex justify-between items-center mb-10">
        ...
        {{-- Kanan: Ikon Manage Lowongan + Search + Notifikasi + User + Logout --}}
        <div class="flex items-center gap-8 text-gray-700">

            {{-- Ikon Kelola Interview --}}
            ...
            {{-- Ikon Cari Kandidat (BARU) --}}
            <a href="{{ route('employer.temukan-kandidat') }}" class="hover:text-blue-600 cursor-pointer"
                title="Cari Kandidat">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 2xl:w-8 2xl:h-8" fill="none"
            ...
            @php
                ...
                $unreadCount = $employer
                    ? EmployerNotification::where('employer_id', $employer->id)->where('is_read', false)->count()
                    : 0;
            @endphp

            <a href="{{ route('employer.notifications') }}" class="relative hover:text-blue-600 cursor-pointer"
                title="Notifikasi">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 2xl:w-8 2xl:h-8 text-gray-700" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                    <path d="M18 8a6 6 0 00-12 0c0 7-3 9-3 9h18s-3-2-3-9" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path d="M13.73 21a2 2 0 01-3.46 0" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
                @if ($unreadCount > 0)
                    <span
                        class="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-bold rounded-full px-1.5 py-0.5">
                        {{ $unreadCount }}
                    </span>
                @endif
            </a>
            ...
            <a href="{{ route('employer.edit-profile', ['slug' => auth()->user()->employer->slug]) }}"
                ...
                @php $photo = auth()->user()->employer->photo_profile; @endphp
                @if ($photo)
                    <img src="{{ $employer->photo_profile === 'images/default_employer.png'
                        ? asset('images/default_employer.png')
                        : asset('storage/' . $employer->photo_profile) }}"
                        alt="Foto Profil"
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="hover:text-red-600 cursor-pointer" title="Logout">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 2xl:w-8 2xl:h-8" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round"
