...
<div x-data="{ show: false }" class="relative">
    <input :type="show ? 'text' : 'password'" name = "{{ $name }}"
        {{ $attributes->merge([
            'class' =>
                'w-full pr-10 border-gray-700 focus:border-[#7397EA] focus:ring-[#7397EA] rounded-md shadow-sm placeholder:opacity-50',
        ]) }} />

    <!-- Ikon mata di dalam input -->
    <button type="button" @click="show = !show" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-600"
        tabindex="-1">
        <!-- Mata tertutup -->
