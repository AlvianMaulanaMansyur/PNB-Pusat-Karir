<aside class="w-64 h-screen fixed top-0 left-0 bg-blue-900 text-white shadow-md flex flex-col overflow-y-auto z-40">
    ...
    <nav class="flex-1 px-4 py-4 space-y-4 font-semibold text-sm uppercase">
        <a href="{{ route('admin.dashboard') }}"
            class="flex items-center gap-2 px-2 py-2 rounded hover:bg-blue-800 transition">
        ...
        <div x-data="{ open: false }">
            <button @click="open = !open"
                ...
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M16.707 9.293a1 1 0 00-1.414 0L11 13.586 8.707 11.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l5-5a1 1 0 000-1.414z" />
                </svg>
                <span>MANAJEMEN AKUN</span>
                <svg class="w-4 h-4 ml-auto transform transition-transform" :class="open ? 'rotate-90' : ''"
        ...
        <form action="{{ route('admin.logout') }}" method="post">
            <button type="submit" class="flex justify-center gap-2 px-2 py-2 rounded text-white bg-red-500 transition">
                Log Out
            </button>
        </form>
    </nav>
